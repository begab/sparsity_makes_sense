...
def calc_inconsistency(X):
    return np.triu(np.abs(X.T @ X), k=1).max()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--short_alphas_name', type=str)
    ...
    dict_file = args.predefined_dictionary_file.replace('.npy', '') if args.predefined_dictionary_file is not None else None # when follows random:x pattern, the dictionary is randomly generated using seed x
    D = None
    if args.spams:
        import spams
    else:
    ...
    for i, in_file in enumerate(args.in_files):
        ...
        if args.center:
            ...
            embeddings -= mu
            np.save(f'{in_file.replace(".npy", "")}_mean', mu)
            logging.info(f'Mu vector of shape {mu.shape} saved to {in_file}.mean')
        if args.normalize:
        ...
        if dict_file is None:
            ...
            print(dict_file)
            if args.center:
                dict_file = f'{dict_file.replace(".npy", "")}_centered'
        ...
        inconsistency = calc_inconsistency(D)
        logging.info((D.dtype, embeddings.dtype, D.shape, embeddings.shape, inconsistency))
       
        args.K = D.shape[1] 
        alphas_file = f'{dict_file}_{args.short_alphas_name if args.short_alphas_name else os.path.basename(in_file)}_norm{args.normalize}_K{args.K}_lda{args.lda}'
