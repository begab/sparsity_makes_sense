...
def calculate_rcsls(R, tgt, src, spectral, batchsize=0, niter=10, knn=10, maxneg=50000, lr=1.0, verbose=False):
    ...
    for it in range(0, niter+1):
        ...
        if spectral:
            ...
            R = U @ (np.diag(s) @ V)
        if verbose:
            logging.info(f"[it={it}] (lr={lr:.4f}) f = {f:.4f}"
...
def main():
    ...
    #logging.info(args)
    out_dir = f'{args.out_dir}{"-pt" if args.pt else ""}/{args.src_pooling}-{args.tgt_pooling}/'
    out_dir_name = os.path.dirname(out_dir)
    if not os.path.exists(out_dir_name):
        os.makedirs(out_dir_name)

    threshold = 25000
    out_file = f'{out_dir}/{args.src_transformer.replace("/", "_")}_{args.tgt_transformer.replace("/", "_")}'
    ...
    if os.path.exists(out_file):
        ...
        for i, tr in enumerate(tatoeba['train']['translation']):
            if i%2500==0 or len(x)>threshold:
                logging.info(f'{i}\t{number_of_translated_sentences}\t{tr}\t{len(x)}')
        ...
        with open(f'{out_file}-{len(x)}.pkl', 'wb') as fo:
    ...
    for l1, X_emb in X.items():
        for l2, Y_emb, in Y.items():
            ...
            if args.pt == True:
                src = torch.tensor(src).float().to(f'cuda:{args.gpu_id}')
                tgt = torch.tensor(tgt).float().to(f'cuda:{args.gpu_id}')
            ...
            for method, trafo in zip(['isometric', 'rcsls', 'identity'], [isometric_trafo, rcsls_trafo, identity_trafo]):
                ...
                trafo_file = f'{out_dir}/{method}_{args.src_transformer.replace("/", "_")}_{l1}_{args.tgt_transformer.replace("/", "_")}_{l2}_{limit}'
                if method != 'identity':
                ...
                for topk in [1]:#, 5, 10, 15]:
                    ...
                    logging.info(f"{method}-{args.src_pooling}-{args.tgt_pooling}\t{args.src_lang}\t{args.tgt_lang}\t{args.scr_transformer}\t{args.tgt_transformer}\t{l1}\t{l2}\t{limit}\t{len(x)-limit}\t{topk}\t{accuracy:.3f}\t{accuracy2:.3f}\t{top_sims_avg:.3f}\t{avg_sims:.3f}\t{diff:.3f}")
