    ...
    for i, in_file in enumerate(args.in_files):
        ...
        for ii, (sequence, is_labeled) in enumerate(p.reader.read_sequences(in_file)):   
            ...
            for token, token_from, token_end, it in zip(sequence, tokens, tokens[1:], is_tagged):
                #logging.info(f"{token_from}\t{token_end}\t{token_end-token_from}")
                #print(f'{args.transformer}\t{token_end-token_from}\t{it}\t{token}\t{in_file}')
        ...
        print(f'fertility\t{mean_fertility}\t{args.transformer}\t{in_file}')
        print(f'MTR\t{multi_token_ratio}\t{args.transformer}\t{in_file}')
        print(f'fertility-L\t{mean_fertility_labeled}\t{args.transformer}\t{in_file}')
        print(f'MTR-L\t{multi_token_ratio_labeled}\t{args.transformer}\t{in_file}')
        for layer in total_mlm_loss:
            print(f'MLM\t{total_mlm_loss[layer] / total_masked_tokens}\t{args.transformer}\t{in_file}\t{layer}')
            print(f'Acc\t{total_corrects[layer] / total_masked_tokens}\t{args.transformer}\t{in_file}\t{layer}')
        
        #logging.info(f'{ii}\t{args.transformer}\t{len(is_tagged)}\t{len(fertility)}\t{np.sum(is_tagged)}')
        #logging.info(f'{args.transformer}\t{in_file}\t{mean_fertility:.3f}')
        #logging.info(f'{args.transformer}\t{in_file}\t{multi_token_ratio:.3f}')
        #logging.info(f'{args.transformer}\t{in_file}\t{mean_fertility_labeled:.3f}')
        #logging.info(f'{args.transformer}\t{in_file}\t{multi_token_ratio_labeled:.3f}')
